...
NameDB::NameDB(string_view nameFile)
{
    ...
    ifstream intputFile(nameFile.data());
    if(!intputFile){
    ...
    // 이름을 한 번에 하나씩 읽는다.
    string name;
    while (intputFile >> name){
        // 주어진 이름이 데이터베이스에 이미 있는지 확인한다.
        if(nameExists(name)){
            // 주어진 이름이 데이터베이스에 있다면 횟수만 증가한다.
            incrementNameCount(name);
        }else{
            // 데이터베이스에 이름이 없다면 새로 추가하고 회수를 1로 지정한다.
            addNewName(name);
        }
    }
}

// 데이터베이스에 이름이 있다면 true를 리턴하고, 그렇지 않으면 false를 리턴한다.
bool NameDB::nameExists(string_view name) const
{
    // 이름을 담은 vector에 대해 루프를 돌면서 주어진 이름을 검색한다.
