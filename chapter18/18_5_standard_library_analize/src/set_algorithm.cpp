...
int main()
{
    ...
    if(includes(cbegin(vec1), cend(vec1), cbegin(vec2), cend(vec2))){
        cout << "The second set is subset of the first."<<endl;
    }
    ...
    DumpRange("The intersaction is ", begin(result), newEnd);
    ...
    DumpRange("The symmetic difference is: ", begin(result), newEnd);
    return 0;
}