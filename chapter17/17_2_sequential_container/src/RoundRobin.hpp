...
#include <stdexcept>
// RoundRobin 클래스 템플릿
// 리스트에 담긴 원소에 대해 라운드 로빈 스케줄링을 구현한다.

template <typename T>
class RoundRobin
{
public:
    // 효율을 높이기 위해 원소의 예상 계수를 클라이언트로부터 받는다.
    ...
    // 리스트의 다음번 원소를 리턴한다. 첫 번째 원소부터 시작해서 마지막
    // 원소에 다다르면 다시 첫 번째 원소로 돌아온다. 이때 중간에 추가되거나
    // 삭제된 원소도 반영한다.
    T& getNext();
private:
...
{
    // 클라이언트가 알려준 만큼의 공간을 확보한다.
    mElements.reserve(numExpected);

    // 원소가 최소 하나 있을 때까지 사용하지 않겠지만,
    // 일단 mCurrentElemetn를 초기화해둔다.
...
void RoundRobin<T>::add(const T& element)
{
    ...
    // vector가 재할당돼서 mCurrentElement 반복자가 무효화될 수 있다.
    ...
    for(auto it = begin(mElements); it != end(mElements); ++ it)
    {
        if(*it==element)
        {
            ...
            if(mCurrentElement == end(mElements) -1 && mCurrentElement == it)
            {
                ...
                //그 원소를 삭제하려 하므로 다시 첫 번째 원소를 위치를 이동한다.
                newPos = 0;
            }
...
{
    // 가장 먼저 원소가 담겨 있는지부터 확인한다.
    if (mElements.empty())
    {
        throw std::out_of_range("No elements in th list");
    }
